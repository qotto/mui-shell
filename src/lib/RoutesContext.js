...
const getExactMatch = pathname => route => matchPath(pathname, { path: route.path, exact: true });
...
const enhanceRoute = (routes, parentPath = "", depth = 0, parentRoute = null) => {
  const enhancedRoute = {
    ...routes,
  ...
  if (routes.children) {
    enhancedRoute.children = routes.children.map(child => enhanceRoute(child, parentPath + routes.path, depth + 1, enhancedRoute));
  }
...
class RoutesProvider extends React.PureComponent {
  ...
  static getDerivedStateFromProps = ({ routes, location }, state) => {
    ...
    const currentMatch = currentRoute && getExactMatch(location.pathname)(currentRoute);
    ...
    const locations = exists ? state.locations : [...state.locations, newLocation];
  ...
  render() {
    return <RoutesContext.Provider value={this.state}>{this.props.children}</RoutesContext.Provider>;
  }
}
