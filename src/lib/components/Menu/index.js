...
const Menu = ({ toggled, openMenu, closeMenu, menuLogoSrc, classes, linkFormatter, menuLabelFormatter, drawerFooter }) => {
      ...
      <List component="div" className={classes.drawerMenu}>
        <RoutesContext.Consumer>
          {({ flatRoutes, currentRoute, parentRoutes }) => {
            const activeRoute = currentRoute && [currentRoute, ...parentRoutes].find(route => route.menu);
            return flatRoutes
              ...
              .map(route => (
                <ListItem
                  ...
                  isActive={() => route.fullPath === (activeRoute && activeRoute.fullPath)}
                  activeClassName={classes.activeMenuItem}
                  button
                  onClick={closeMenu}
                >
                  <ListItemIcon>{route.menu.icon}</ListItemIcon>
                  <ListItemText primary={menuLabelFormatter ? menuLabelFormatter(route) : route.label} />
                </ListItem>
              ));
          }}
        </RoutesContext.Consumer>
